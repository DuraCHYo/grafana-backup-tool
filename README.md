...
docker run --network=host --rm --name grafana-backup-tool \
  ...
  ghcr.io/durachyo/grafana-backup-tool:v1.6.5 save
```
#### Save to S3
```bash
docker run --network=host --rm --name grafana-backup-tool \
...
docker run --network=host --rm --name grafana-backup-tool \
  ...
  -e GRAFANA_URL="http://grafana:3000" \
  -v /tmp/backup:/opt/grafana-backup-tool/_OUTPUT_ \
  ghcr.io/durachyo/grafana-backup-tool:v1.6.5 restore _OUTPUT_/2026-03-29-00-18.tar.gz
```
#### Restore from S3
```bash
docker run --network=host --rm --name grafana-backup-tool \
  ...
  -e AWS_SECRET_ACCESS_KEY=d084c2836e8e38bc0cf8ea9db915d15c0b8dad6ddd40219bdb733953f4b452da \
  -e AWS_ENDPOINT_URL=http://localhost:3900 \
  ghcr.io/durachyo/grafana-backup-tool:v1.6.5  restore 2026-03-29-00-18.tar.gz
```
