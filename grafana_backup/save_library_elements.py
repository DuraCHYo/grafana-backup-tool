...
def main(args, settings):
    ...
    get_individual_library_elements_and_save(
        library_elements, folder_path, log_file, pretty_print
    )
    print_horizontal_line()


def get_all_library_elements_in_grafana(
    grafana_url, http_get_headers, verify_ssl, client_cert, debug
):
    (status, content) = search_library_elements(
        grafana_url, http_get_headers, verify_ssl, client_cert, debug
    )
    if status == 200:
        ...
        for library_element in library_elements:
            print("name: {0}".format(library_element["name"]))
        return library_elements
    else:
        print(
            "query library elements failed, status: {0}, msg: {1}".format(
                status, content
            )
        )
        return []
...
def save_library_element(
    channel_name, file_name, alert_channel_setting, folder_path, pretty_print
):
    file_path = save_json(
        file_name, alert_channel_setting, folder_path, "library_element", pretty_print
    )
        ...
        with open("{0}".format(file_path), "w") as f:
            for library_element in library_elements:
                ...
                save_library_element(
                    ...
                    str(library_element_identifier),
                    library_element,
                    folder_path,
                    pretty_print,
                )
