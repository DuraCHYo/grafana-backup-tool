...
def main(args, settings, file_path):
    ...
    if http_get_headers_basic_auth:
        ...
        user_id = get_user_by_email_or_username(
            urllib.parse.quote(team_member["email"]),
            grafana_url,
        ...
        if 200 != user_id[0]:
            user_id = get_user_by_email_or_username(
                urllib.parse.quote(team_member["name"]),
                grafana_url,
                http_get_headers_basic_auth,
                verify_ssl,
                client_cert,
        ...
        print(
            "[ERROR] Restoring team members needs to set GRAFANA_ADMIN_ACCOUNT and GRAFANA_ADMIN_PASSWORD first. \n"
        )
