...
def get_all_datasources_and_delete(
    grafana_url,
    http_get_headers,
    verify_ssl,
    client_cert,
    ...
    if status_code_and_content[0] == 200:
        ...
        for datasource in datasources:
            print(datasource)
            if uid_support:
                status = delete_datasource_by_uid(
                    datasource["uid"],
                    grafana_url,
                    http_get_headers,
                    verify_ssl,
                ...
                status = delete_datasource_by_id(
                    datasource["id"],
                    grafana_url,
                    http_get_headers,
                    verify_ssl,
            ...
            if status == 200:
                ...
                print(
                    "deleting of datasource {0} failed with: {1}".format(
                        datasource["name"], status
                    )
                )
    else:
        print(
            "query datasource failed, status: {0}, msg: {1}".format(
