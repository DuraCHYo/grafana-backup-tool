...
def main(args, settings):
    ...
    if paging_support:
        delete_dashboards_above_Ver6_2(
            grafana_url,
            http_get_headers,
            verify_ssl,
            client_cert,
...
def get_individual_dashboard_and_delete(
    dashboards,
    grafana_url,
    http_get_headers,
    verify_ssl,
    ...
    if dashboards:
        for board in dashboards:
            if uid_support:
                status = delete_dashboard_by_uid(
                    board["uid"],
                    grafana_url,
                    http_get_headers,
                    verify_ssl,
                ...
                status = delete_dashboard_by_slug(
                    board["slug"],
                    grafana_url,
                    http_get_headers,
                    verify_ssl,
            ...
            if status == 200:
                ...
                print(
                    "got {0} when trying to delete board {1}".format(
                        status, board["title"]
                    )
                )


def delete_dashboards_above_Ver6_2(
    grafana_url,
    http_get_headers,
    verify_ssl,
    client_cert,
