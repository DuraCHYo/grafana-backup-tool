...
from grafana_backup.dashboardApi import get_org, search_orgs


@register_component("save", "organizations")
...
def save_org_info(org_name, file_name, org_settings, folder_path, pretty_print):
    file_path = save_json(
        file_name, org_settings, folder_path, "organization", pretty_print
    )
    print("org: {0} -> saved to: {1}".format(org_name, file_path))


def get_individual_org_info_and_save(
    orgs,
    folder_path,
    log_file,
    grafana_url,
    ...
    if orgs:
        with open("{0}".format(file_path), "w") as log_file:
            for org in orgs:
                (status, content) = get_org(
                    org["id"],
                    grafana_url,
                    http_get_headers,
                    verify_ssl,
                ...
                if status == 200:
                    save_org_info(
                        org["name"], str(org["id"]), content, folder_path, pretty_print
                    )
...
def save_orgs(
    ...
    orgs = get_all_orgs_in_grafana(
        grafana_url, http_get_headers, verify_ssl, client_cert, debug
    )
    print_horizontal_line()
    get_individual_org_info_and_save(
        orgs,
        folder_path,
        log_file,
        grafana_url,
