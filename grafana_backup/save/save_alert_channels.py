...
@register_component("save", "alert-channels")
def main(args, settings):
    ...
    get_individual_alert_channel_and_save(
        alert_channels, folder_path, log_file, pretty_print
    )
...
def save_alert_channel(
    ...
    file_path = save_json(
        file_name, alert_channel_setting, folder_path, "alert_channel", pretty_print
    )
    print("alert_channel:{0} is saved to {1}".format(channel_name, file_path))


def get_individual_alert_channel_and_save(
    channels, folder_path, log_file, pretty_print
):
    ...
    if channels:
        with open("{0}".format(file_path), "w") as f:
            for channel in channels:
                ...
                f.write("{0}\t{1}\n".format(str(channel_identifier), channel["name"]))
