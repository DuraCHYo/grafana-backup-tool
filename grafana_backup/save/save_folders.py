...
def save_folder_setting(
    ...
    file_path = save_json(
        file_name, folder_settings, folder_path, "folder", pretty_print
    )
    ...
    file_path = save_json(
        file_name, folder_permissions, folder_path, "folder_permission", pretty_print
    )
...
def get_individual_folder_setting_and_save(
    ...
    with open("{0}".format(file_path), "w+") as f:
        for folder in folders:
            ...
            (status_folder_settings, content_folder_settings) = get_folder(
                folder["uid"],
                grafana_url,
                http_get_headers,
                verify_ssl,
            ...
            (status_folder_permissions, content_folder_permissions) = (
                get_folder_permissions(
                    folder["uid"],
                    grafana_url,
                    http_get_headers,
