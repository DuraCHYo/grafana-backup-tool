...
def get_all_dashboards_in_grafana(
    ...
    if status == 200:
        ...
        for board in dashboards:
            print("name: {0}".format(board["title"]))
        return dashboards
    else:
...
def save_dashboard_setting(
    dashboard_name, file_name, dashboard_settings, folder_path, pretty_print
):
    file_path = save_json(
        file_name, dashboard_settings, folder_path, "dashboard", pretty_print
    )
            ...
            for board in dashboards:
                ...
                if status == 200:
                    file_name = build_filename(
                        board_uri, content, uid_support, slug_suffix
                    )
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for future in as_completed(future_to_board):
            try:
                ...
                board_uri = (
                    "uid/{0}".format(board["uid"])
                    if uid_support
                    else board.get("uri", "unknown")
                )
                print(
                    "Dashboard {0} generated an exception: {1}".format(board_uri, exc)
                )
