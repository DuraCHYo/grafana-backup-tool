...
def get_single_snapshot_and_save(
    snapshot,
    grafana_url,
    http_get_headers,
    verify_ssl,
    ...
    (status, content) = get_snapshot(
        snapshot["key"], grafana_url, http_get_headers, verify_ssl, client_cert, debug
    )
    if status == 200:
...
def get_all_snapshots_and_save(
    folder_path,
    grafana_url,
    http_get_headers,
    verify_ssl,
    ...
    status_code_and_content = search_snapshot(
