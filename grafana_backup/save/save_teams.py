...
def save_team(
    ...
    file_path = save_json(
        file_name, alert_channel_setting, folder_path, "team", pretty_print
    )
    ...
    if teams:
        with open("{0}".format(file_path), "w") as f:
            for team in teams:
                ...
                save_team(
                    team["name"], str(team_identifier), team, folder_path, pretty_print
                )
                f.write("{0}\t{1}\n".format(str(team_identifier), team["name"]))
