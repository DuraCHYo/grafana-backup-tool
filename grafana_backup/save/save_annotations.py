...
@register_component("save", "annotations")
...
def save_annotation(file_name, annotation_setting, folder_path, pretty_print):
    file_path = save_json(
        file_name, annotation_setting, folder_path, "annotation", pretty_print
    )
    print("annotation: {0} is saved to {1}".format(file_name, file_path))


def get_all_annotations_and_save(
    folder_path,
    grafana_url,
    http_get_headers,
    verify_ssl,
    ...
    while ts_from > thirteen_months_retention:
        ...
        if status_code_and_content[0] == 200:
            ...
            for annotation in annotations_batch:
                print(annotation)
                save_annotation(
                    str(annotation["id"]), annotation, folder_path, pretty_print
                )
        else:
            print(
