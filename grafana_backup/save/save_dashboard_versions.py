...
from .save_dashboards import get_all_dashboards_in_grafana


@register_component("save", "dashboard-versions")
...
def get_versions_and_save(
    ...
    if dashboards:
        for board in dashboards:
            ...
            (status, content) = get_dashboard_versions(
                board["id"],
                grafana_url,
                http_get_headers,
                verify_ssl,
    ...
    if versions:
        with open("{0}".format(file_path), "w") as f:
            for version in versions:
                ...
                if status == 200:
                    save_version(
                        str(version["version"]), content, folder_path, pretty_print
                    )
