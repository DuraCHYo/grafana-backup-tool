...
from grafana_backup.dashboardApi import get_user, get_user_org, search_users


@register_component("save", "users")
...
def get_individual_user_and_save(
    users,
    folder_path,
    log_file,
    grafana_url,
    ...
    if users:
        with open("{0}".format(file_path), "w") as f:
            for user in users:
                ...
                save_user_info(
                    user["name"], str(user["id"]), user, folder_path, pretty_print
                )
